...
create or replace transient table DEV.CHEDDR_WEDDR.fct__satellite_radiation
    
  (
    location_id number not null references DEV.CHEDDR_WEDDR.dim__locations (location_id),
    ts_time timestamp_ntz not null,
    field varchar not null references DEV.CHEDDR_WEDDR.dim__satellite_radiation_units (field),
    value float,
    data_date date,
